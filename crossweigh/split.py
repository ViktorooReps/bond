...
from bond.data import DatasetName, load_tags_dict
...
def load_dataset_from_json(json_file: str, tag_to_id: Dict[str, int]) -> List[Tuple[List[str], List[str]]]:
    ...
    for doc in json_dataset:
        for sent in doc:
            tokens: List[str] = list(sent['str_words'])
            labels: List[str] = [id_to_tag[tag_id] for tag_id in sent['tags']]
            sentences.append((tokens, labels))
